...
test.describe("Public Mobile Foundation", () => {
  for (const route of ["/staff/login", "/owner/login", "/shopify/login", "/shopify-admin/login", "/book"]) {
    test(`${route} has no root horizontal overflow`, async ({ page }) => {
      await page.goto(route, { waitUntil: "networkidle" });
      expect(await hasHorizontalOverflow(page)).toBe(false);
    });

    test(`${route} app hosts fit viewport`, async ({ page }) => {
      await page.goto(route, { waitUntil: "networkidle" });
      const result = await page.evaluate(() => {
        const selectors = ["html", "body", "aura-root", "ion-app", "ion-router-outlet"];
        return selectors.map((selector) => {
          const el = document.querySelector(selector);
          const rect = el?.getBoundingClientRect();
          return {
            selector,
            ok: !rect || (rect.left >= -2 && rect.right <= window.innerWidth + 2),
            left: rect?.left ?? 0,
            right: rect?.right ?? 0,
            width: rect?.width ?? 0,
      ...
      expect(result.filter((item) => !item.ok), JSON.stringify(result)).toHaveLength(0);
    });

    test(`${route} mobile inputs avoid iOS zoom sizing`, async ({ page }) => {
      await page.goto(route, { waitUntil: "networkidle" });
      const sizes = await page.evaluate(() => Array.from(document.querySelectorAll("input, textarea, select"))
        .filter((el) => {
          const rect = el.getBoundingClientRect();
          const style = getComputedStyle(el);
          return rect.width > 0 && rect.height > 0 && style.visibility !== "hidden" && style.display !== "none";
        })
        .map((el) => ({ tag: el.tagName.toLowerCase(), fontSize: parseFloat(getComputedStyle(el).fontSize) })));

      for (const item of sizes) {
        expect(item.fontSize, `${route} ${item.tag} font-size is below 16px`).toBeGreaterThanOrEqual(16);
      }
    });

    test(`${route} mobile text stays compact`, async ({ page }) => {
      await page.goto(route, { waitUntil: "networkidle" });
      const oversized = await page.evaluate(() => Array.from(document.querySelectorAll("h1, h2, h3, p, label, button, a, strong, small, span"))
        .filter((el) => {
          const rect = el.getBoundingClientRect();
          const style = getComputedStyle(el);
          const text = (el.textContent || "").trim();
          return text.length > 1 && rect.width > 0 && rect.height > 0 && style.visibility !== "hidden" && style.display !== "none";
        })
        .map((el) => ({
        ...
        .filter((item) => item.fontSize > 38));

      expect(oversized, JSON.stringify(oversized)).toHaveLength(0);
    });

    test(`${route} visible text does not spill horizontally`, async ({ page }) => {
      await page.goto(route, { waitUntil: "networkidle" });
      const spill = await page.evaluate(() => Array.from(document.querySelectorAll("h1, h2, h3, p, label, button, a, strong, small, span"))
        .filter((el) => {
          const rect = el.getBoundingClientRect();
          const style = getComputedStyle(el);
          const text = (el.textContent || "").trim();
        ...
        .filter((item) => item.rectRight > item.viewport + 2 || item.scrollWidth > item.clientWidth + 3));

      expect(spill, JSON.stringify(spill)).toHaveLength(0);
    });

    test(`${route} mobile widgets stay phone-sized`, async ({ page }) => {
      await page.goto(route, { waitUntil: "networkidle" });
      const tallWidgets = await page.evaluate(() => Array.from(document.querySelectorAll(".panel, .card, .login-card, .staff-card, .sb-card, .owner-login-story, .owner-login-panel form"))
        .filter((el) => {
          const rect = el.getBoundingClientRect();
          const style = getComputedStyle(el);
          return rect.width > 0 && rect.height > 0 && style.visibility !== "hidden" && style.display !== "none";
        ...
        .filter((item) => item.height > item.viewportHeight * .92));

      expect(tallWidgets, JSON.stringify(tallWidgets)).toHaveLength(0);
    });
  }
});
