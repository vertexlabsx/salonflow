  ...
  styles: [`
    ...
    .mobile-bottom-nav { display: none; }
     @media (max-width: 900px) {
       ...
       .menu-button { display: inline-flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; flex: 0 0 44px; width: 44px; height: 44px; margin: 0; padding: 0; border: 1px solid color-mix(in srgb,var(--staff-primary) 12%,var(--staff-border)); border-radius: 15px; background: var(--staff-surface); color: var(--staff-text); font-size: .78rem; font-weight: 750; box-shadow: 0 8px 18px rgba(42,20,51,.05); }
      ...
      .bell-icon { width: 19px; height: 19px; }
       ...
       .staff-policy-hint { margin: 0 10px 8px; padding: 7px 10px; font-size: .72rem; line-height: 1.3; }
        .notification-drawer { top: 0; right: 0; bottom: 0; left: auto; width: min(420px, 92vw); min-width: 280px; max-width: none; height: 100dvh; padding: calc(14px + env(safe-area-inset-top)) calc(14px + env(safe-area-inset-right)) calc(14px + env(safe-area-inset-bottom)) calc(14px + env(safe-area-inset-left)); border-left: 1px solid var(--staff-border); border-radius: 26px 0 0 26px; box-shadow: -22px 0 54px rgba(42,20,51,.2); }
      .notification-drawer .drawer-title { position: sticky; top: 0; z-index: 2; border: 1px solid var(--staff-border); border-radius: 16px; background: var(--staff-surface-secondary); box-shadow: 0 6px 16px rgba(31, 41, 55, .08); }
       ...
       .mobile-bottom-nav:has(> a:nth-child(5)) { grid-template-columns: repeat(auto-fit, minmax(0, 1fr)); gap: 1px; }
        .mobile-bottom-nav a { position:relative;display: grid; grid-template-columns: 1fr; grid-template-rows: 23px auto; place-items: center; align-content: center; gap: 2px; min-width: 0; min-height:56px; padding: 6px 2px; border: 0; border-radius: 16px; color: var(--staff-text-secondary); font-size: clamp(.52rem, 2.55vw, .62rem); font-weight: 700; line-height: 1; text-decoration: none; transition:transform var(--staff-motion-fast) var(--staff-motion-ease),opacity var(--staff-motion-fast) var(--staff-motion-ease); } .mobile-bottom-nav a span, .mobile-bottom-nav a.active span { display: block; max-width: 100%; width: auto; height: auto; padding: 0; border: 0; border-radius: 0; background: transparent; color: inherit; font-size: inherit; font-weight: inherit; letter-spacing: -.02em; text-transform: none; overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }
      ...
      nav a { padding: 12px 13px; }
       ...
       @media (max-width: 380px) {
          ...
          .mobile-bottom-nav a { min-height:52px;font-size:.5rem; }
