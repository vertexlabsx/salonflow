...
const BOOKING_KEYWORDS = ["book", "book appointment", "appointment", "i want to book", "need appointment", "book_appointment", "book now", "new appointment"];
...
async function bookingFlowInteractive(salonId: string, waPhone: string, branches: Array<{ _id: string; name: string }> = [], body = "Choose department, branch, date and time in one smooth WhatsApp form."): Promise<Record<string, unknown> | null> {
  ...
  if (env.WHATSAPP_BOOKING_FLOW_LAYOUT === "guided") {
    return {
      type: "flow",
      ...
      action: {
        ...
        parameters: {
          ...
          flow_action_payload: {
            ...
            data: {
              salonId,
              branch: branches.slice(0, 10).map((branch) => ({ id: branch._id, title: branch.name }))
            }
          }
        }
      }
...
async function whatsappFlowDataResponse(payload: Record<string, unknown>): Promise<{ version: string; screen: string; data: Record<string, unknown> }> {
  ...
  if (screen === "BRANCH") {
    const branch = flowString(data, ["branch"]);
    return { version, screen: "CATEGORY", data: { salonId, branch, category: await guidedFlowCategories(salonId, branch) } };
  }

  if (screen === "CATEGORY") {
    const branch = flowString(data, ["branch"]);
    const category = flowString(data, ["category"]);
    return { version, screen: "SERVICE", data: { salonId, branch, category, service: await guidedFlowServices(salonId, branch, category) } };
  }

  if (screen === "SERVICE") {
    const branch = flowString(data, ["branch"]);
    const category = flowString(data, ["category"]);
    const service = flowString(data, ["service"]);
    return { version, screen: "STAFF", data: { salonId, branch, category, service, staff: await guidedFlowStaff(salonId, branch, service) } };
  }

  if (screen === "STAFF") {
    return { version, screen: "DATE_TIME", data: await guidedFlowDateTime(salonId, data) };
  }

  if (screen === "DATE_TIME") {
    if (String(data.trigger || "") === "date_selected") return { version, screen: "DATE_TIME", data: await guidedFlowDateTime(salonId, data) };
    return { version, screen: "DETAILS", data: { salonId, branch: flowString(data, ["branch"]), category: flowString(data, ["category"]), service: flowString(data, ["service"]), staff: flowString(data, ["staff"]), date: flowString(data, ["date"]), time: flowString(data, ["time"]) } };
  }
  ...
  const guidedFields = {
    branch: flowString(data, ["branch"]),
    category: flowString(data, ["category"]),
    service: flowString(data, ["service"]),
    staff: flowString(data, ["staff"]),
    date: flowString(data, ["date"]),
    time: flowString(data, ["time"]),
    name: fields.name,
    email: fields.email,
    phone: fields.phone,
  ...
  if (screen === "DETAILS") {
    if (guidedFields.branch || guidedFields.service || guidedFields.staff) {
      const appointment = await guidedFlowAppointmentText(salonId, guidedFields);
      return { version, screen: "SUMMARY", data: { appointment, details: buildFlowDetailsText(guidedFields), salonId, ...guidedFields } };
    }
  ...
  if (screen === "SUMMARY") {
    const bookingFields = guidedFields.branch || guidedFields.service || guidedFields.staff ? { ...guidedFields, branchId: guidedFields.branch, serviceIds: [guidedFields.service], staffId: guidedFields.staff } : fields;
    const booking = await finalizeFlowBooking(salonId, waPhone || fields.phone, fields.name, "", bookingFields as Record<string, unknown>);
...
async function guidedFlowCategories(salonId: string, branchId: string): Promise<Array<{ id: string; title: string }>> {
  if (!branchId) return [];
  const docs = await ServiceModel.find(branchServiceFilter(salonId, branchId)).select("category name").sort({ category: 1, name: 1 }).limit(100);
  return [...new Set(docs.map((service) => service.category || "Services"))].slice(0, 10).map((category) => ({ id: category, title: category.slice(0, 30) }));
}

async function guidedFlowServices(salonId: string, branchId: string, category: string): Promise<Array<{ id: string; title: string; description?: string }>> {
  if (!branchId || !category) return [];
  const docs = await ServiceModel.find({ ...branchServiceFilter(salonId, branchId), $or: [{ category }, ...(category === "Services" ? [{ category: { $in: [null, ""] } }] : [])] }).sort({ name: 1 }).limit(10);
  return docs.map((service) => ({ id: String(service._id), title: service.name.slice(0, 30), description: `${money(service.pricePaise)}, ${service.durationMinutes} min`.slice(0, 72) }));
}

async function guidedFlowStaff(salonId: string, branchId: string, serviceId: string): Promise<Array<{ id: string; title: string }>> {
  if (!branchId || !serviceId) return [];
  const services = await selectedServices({ salonId, branchId, serviceIds: [serviceId] });
  const staff = services.length ? await eligibleStaffForServices(salonId, branchId, services) : [];
  return staff.slice(0, 10).map((member) => ({ id: member.staffId, title: member.name.slice(0, 30) }));
}

async function guidedFlowDateTime(salonId: string, data: Record<string, unknown>): Promise<Record<string, unknown>> {
  const branch = flowString(data, ["branch", "branchId", "location"]);
  const service = flowString(data, ["service", "department"]);
  const staff = flowString(data, ["staff"]);
  const date = flowString(data, ["date"]);
  const dateOptions = Array.from({ length: 14 }, (_, index) => {
    const value = new Date(Date.now() + index * 24 * 60 * 60_000).toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" });
    return { id: value, title: displayDate(value) };
  ...
  let timeOptions: Array<{ id: string; title: string }> = [];
  if (branch && service && staff && date) {
    const docs = await selectedServices({ salonId, branchId: branch, serviceIds: [service] });
    const summary = summarizeServices(docs);
    const slots = summary.duration ? await smartSlotOptions({ salonId, branchId: branch, staff: [{ staffId: staff, name: await staffNameOf(salonId, staff) }], date, durationMinutes: summary.duration, preference: null, limit: 16 }) : [];
    timeOptions = slots.map((slot) => ({ id: slot.label, title: slot.label }));
  }
  return { salonId, branch, category: flowString(data, ["category"]), service, staff, date, date_options: dateOptions, is_time_enabled: timeOptions.length > 0, time: timeOptions };
}

async function guidedFlowAppointmentText(salonId: string, input: { branch: string; service: string; staff: string; date: string; time: string }): Promise<string> {
  const [branch, service] = await Promise.all([
    input.branch ? BranchModel.findOne({ _id: input.branch, salonId }).select("name").lean() : null,
    input.service ? ServiceModel.findOne({ _id: input.service, salonId }).select("name pricePaise durationMinutes").lean() : null
  ]);
  const staff = input.staff ? await staffNameOf(salonId, input.staff) : "Staff";
  return `${service?.name || "Service"}\nBranch: ${branch?.name || "Branch"}\nStaff: ${staff}\nDate: ${input.date ? displayDate(input.date) : "-"}${input.time ? ` at ${input.time}` : ""}`;
}
...
async function handleBookingMessage(salonId: string, branchId: string, message: WaInboundMessage): Promise<Record<string, unknown>> {
  ...
  const isSimpleGreeting = /^(hi|hii|hello|hey|hlo|namaste|namaskar)$/i.test(text);
  ...
  if (!session || session.expiresAt < new Date() || BOOKING_KEYWORDS.includes(lower) || isSimpleGreeting) {
...
async function handleManagementState(ctx: ManagementContext, command: string | null): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    switch (session.state) {
      case "menu": {
        if (/^(hi|hii|hello|hey|hlo|namaste|namaskar)$/.test(lower)) {
          const repeatGreeting = "Hi! I can help you book or manage your appointments.\n\n";
          const flowInteractive = await bookingFlowInteractive(salonId, phone, branches, `${repeatGreeting}Choose department, branch, date and time in one smooth WhatsApp form.`);
          await setSession({ state: "menu", managementAction: null, targetAppointmentId: null, modifyField: null, categoryPage: 0, servicePage: 0, staffPage: 0 });
          if (flowInteractive) {
            const followUp = mainMenuPayload();
            return { ...followUp, action: "booking_flow", reply: `${repeatGreeting}Tap below to book your appointment in one smooth WhatsApp form.`, interactive: flowInteractive, followUp: { ...followUp, action: "menu" } };
          }
...
const receiveWebhook = asyncHandler(async (req, res) => {
    ...
    const primaryInteractive = (result.interactive as Record<string, unknown> | undefined) || null;
    const followUpInteractive = (followUp?.interactive as Record<string, unknown> | undefined) || null;
    if (result.reply) {
      await sendWhatsAppMessage({
        salonId: String(salonId),
        toPhone: normalizedPhone,
        ...
        interactive: primaryInteractive,
    ...
    if (result.followUp && followUp?.reply) {
      await sendWhatsAppMessage({
        ...
        interactive: followUpInteractive,
