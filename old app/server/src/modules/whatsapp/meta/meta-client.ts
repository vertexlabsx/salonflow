...
export async function fetchSharedWabas(accessToken: string): Promise<Array<{ id: string; name?: string }>> {
  try {
    const payload = await graph<{ data?: Array<{ id: string; name?: string }> }>("/me/shared_whatsapp_business_accounts", accessToken);
    return payload.data || [];
  } catch {
    return [];
  }
