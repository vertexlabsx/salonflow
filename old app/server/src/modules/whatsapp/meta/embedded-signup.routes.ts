...
import { exchangeEmbeddedSignupCode, fetchSharedWabas, fetchWabaPhoneNumbers, subscribeWabaToWebhooks } from "./meta-client";
...
embeddedSignupRouter.post("/embedded-signup/callback", requirePermissions({ every: WRITE }), asyncHandler(async (req, res) => {
  ...
  if (!wabaId) {
    const sharedWabas = await fetchSharedWabas(token.access_token);
