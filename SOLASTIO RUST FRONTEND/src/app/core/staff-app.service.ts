...
const STAFF_REFRESH_TOKEN_KEY = "auraStaffRustRefreshToken";
...
export class StaffAppService {
  ...
  private restoreInitialSessionSync(): void {
    try {
      ...
      if (stored.user && typeof stored.user === "object") {
        ...
        this.refreshTokenValue = this.readStoredRefreshToken();
  ...
  async login(payload: { tenantId: string; loginId: string; password: string; branchId?: string }): Promise<StaffUser> {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        const csrfData = this.isCrossOriginApi() ? {} : await this.fetchCsrfToken();
        const loginResponse = await fetch(`${this.baseUrl}/auth/login`, {
          method: "POST",
          credentials: this.isCrossOriginApi() ? "include" : "same-origin",
  ...
  async logout(): Promise<void> {
    try {
      ...
      await CapacitorHttp.post({
        ...
        data: { ...(this.refreshTokenValue ? { refreshToken: this.refreshTokenValue } : {}) }
      });
    } catch {
  ...
  private isHttpStatus(error: unknown, status: number): boolean {
    return error instanceof HttpErrorResponse ? error.status === status : Number((error as { status?: unknown } | null)?.status) === status;
  }

  private async enterpriseOsFallback(query: Record<string, string>): Promise<StaffEnterpriseOs> {
    const [dashboard, today] = await Promise.all([
      this.dashboard(query).catch(() => this.readStoredData<StaffDashboard>("dashboard") || null),
      this.today(query["date"] || staffBusinessDate()).catch(() => null)
    ]);
    const staff = dashboard?.staff || this.profile();
    const appointments = dashboard?.appointments || dashboard?.todayAppointments || [];
    ...
    return {
      staff: staff || { id: "", fullName: this.user()?.name || "Staff", firstName: this.user()?.name || "Staff", lastName: "", mobile: "", email: this.user()?.email || "", roleId: "", department: "", designation: "", status: "active" },
      home: {
        greeting: "Welcome back",
        todayAppointments: dashboard?.summary?.todayAppointments || appointments.length || 0,
        expectedRevenue: dashboard?.summary?.appointmentValue || dashboard?.summary?.revenue || 0,
        tasks: tasks.length,
        pendingPayments: 0,
        recentNotifications: 0,
        targetProgress: { label: "Monthly target", targetValue: 0, achievedValue: 0, percentage: 0, remaining: 0 }
      },
      ...
      serviceTimers: [],
      performance: { revenue: dashboard?.summary?.revenue || 0, completedServices: dashboard?.summary?.completedAppointments || 0, avgUtilization: 0, avgRating: 0, productivityScore: 0, strengths: [], opportunities: [] },
      leaderboard: [],
      gamification: { points: 0, level: 1, stars: 0, dailyStreak: 0, monthlyStreak: 0, badges: [] },
      notifications: [],
      tasks,
      calendar: (today?.schedules || []).map((item) => ({ id: item.id, date: item.scheduleDate, startTime: item.startTime, endTime: item.endTime, type: item.shiftType, status: item.status })),
      reports: {}
    };
  }
  ...
  private saveSession(session: StaffLoginResponse, tenantId: string) {
    ...
    this.refreshTokenValue = session.refreshToken || this.refreshTokenValue;
    this.tenantIdValue = tenantId;
    this.sessionIdValue = crypto.randomUUID();
    this.profile.set(null);
    this.user.set(this.normalizeUser(session.user));
    this.writeStoredRefreshToken(this.refreshTokenValue);
    ...
    this.refreshPromise = (async () => {
      ...
      try {
        ...
        const response = await CapacitorHttp.post({
          ...
          data: {
            ...(this.refreshTokenValue || this.readStoredRefreshToken() ? { refreshToken: this.refreshTokenValue || this.readStoredRefreshToken() } : {}),
            device: { type: "staff-app", name: "Solastio Staff", platform: Capacitor.getPlatform() }
          }
        });
        ...
        this.accessTokenValue = session.accessToken;
        this.refreshTokenValue = session.refreshToken || this.refreshTokenValue || this.readStoredRefreshToken();
  ...
  private isCrossOriginApi(): boolean {
    if (typeof window === "undefined") return false;
    try { return new URL(this.baseUrl, window.location.origin).origin !== window.location.origin; } catch { return false; }
  }

  private async fetchCsrfToken(): Promise<{ csrfToken?: string; token?: string }> {
    const csrfResponse = await fetch(`${this.baseUrl}/auth/csrf`, { credentials: "same-origin" });
    const csrfEnvelope = await csrfResponse.json();
    return this.unwrap<{ csrfToken?: string; token?: string }>(csrfEnvelope);
  }
  ...
  private readStoredRefreshToken(): string {
    try { return sessionStorage.getItem(STAFF_REFRESH_TOKEN_KEY) || ""; } catch { return ""; }
  }

  private writeStoredRefreshToken(token: string): void {
    try { token ? sessionStorage.setItem(STAFF_REFRESH_TOKEN_KEY, token) : sessionStorage.removeItem(STAFF_REFRESH_TOKEN_KEY); } catch { /* Session still works in memory. */ }
  }
