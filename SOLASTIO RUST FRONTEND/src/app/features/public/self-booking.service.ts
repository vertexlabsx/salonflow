...
export class SelfBookingService {
  ...
  branches(salonId: string): Promise<{ branches: PublicBranch[] }> {
    return firstValueFrom(this.http.get<{ branches: PublicBranch[] } | ApiEnvelope<{ branches: PublicBranch[] }>>(this.url("/branches"), { params: { salonId } })).then((response) => this.unwrap(response));
  }

  services(salonId: string, branchId: string): Promise<{ services: PublicService[] }> {
    return firstValueFrom(this.http.get<{ services: PublicService[] } | ApiEnvelope<{ services: PublicService[] }>>(this.url("/services"), { params: { salonId, branchId } })).then((response) => this.unwrap(response));
  }

  staff(salonId: string, branchId: string, serviceId?: string): Promise<{ staff: PublicStaff[] }> {
    return firstValueFrom(this.http.get<{ staff: PublicStaff[] } | ApiEnvelope<{ staff: PublicStaff[] }>>(this.url("/staff"), { params: { salonId, branchId, ...(serviceId ? { serviceId } : {}) } })).then((response) => this.unwrap(response));
  }
  ...
  slots(salonId: string, branchId: string, serviceId: string, date: string, staffId?: string, maxSlots?: number): Promise<{ date: string; slots: PublicSlot[] }> {
    ...
    return firstValueFrom(this.http.get<{ date: string; slots: PublicSlot[] } | ApiEnvelope<{ date: string; slots: PublicSlot[] }>>(this.url("/slots"), { params })).then((response) => this.unwrap(response));
  }
  ...
  availableDates(salonId: string, branchId: string, serviceId: string, fromDate?: string, count?: number): Promise<{ dates: PublicDate[] }> {
    ...
    return firstValueFrom(this.http.get<{ dates: PublicDate[] } | ApiEnvelope<{ dates: PublicDate[] }>>(this.url("/available-dates"), { params })).then((response) => this.unwrap(response));
  }
  ...
  book(payload: {
    ...
    return firstValueFrom(this.http.post<{
      ...
      bookingId: string; staffId: string; serviceId: string; startAt: string; endAt: string;
      status: string; depositApplied: boolean; paymentLink: string; timezone: string; branchName: string;
    }>>(this.url("/book"), payload)).then((response) => this.unwrap(response));
  }

  cancel(salonId: string, appointmentId: string, phone: string): Promise<{ bookingId: string; status: string; previousStartAt: string }> {
    return firstValueFrom(this.http.post<{ bookingId: string; status: string; previousStartAt: string } | ApiEnvelope<{ bookingId: string; status: string; previousStartAt: string }>>(this.url("/cancel"), { salonId, appointmentId, phone })).then((response) => this.unwrap(response));
  }
  ...
  reschedule(salonId: string, appointmentId: string, phone: string, newStartAt: string, branchId?: string): Promise<{ bookingId: string; status: string; newStartAt: string; timezone: string }> {
    ...
    return firstValueFrom(this.http.post<{ bookingId: string; status: string; newStartAt: string; timezone: string } | ApiEnvelope<{ bookingId: string; status: string; newStartAt: string; timezone: string }>>(this.url("/reschedule"), body)).then((response) => this.unwrap(response));
  }
}
