...
@Component({
    ...
    <section class="shopify-auto-page">
      ...
      @if (activeTab() === 'Flows') {
        <section class="split">
          ...
          <article class="card flow-canvas"><h2>Selected flow</h2>@if (selectedFlow()) { @for (node of selectedFlow()!.nodes; track node.id) { <div class="node" [attr.data-type]="node.type"><span>{{ node.type }}</span><strong>{{ node.label }}</strong><small>{{ node.config | jsonText }}</small>@if (node.type === 'whatsapp_template') { <button type="button" (click)="beginTest(selectedFlow()!, node.id); $event.stopPropagation()">Send Test</button>@if(testNode()?.flowId === selectedFlow()!._id && testNode()?.nodeId === node.id){<div class="test-box"><input [(ngModel)]="testPhoneInput" placeholder="Phone with country code" (keydown.enter)="sendTest(selectedFlow()!, node.id)"><button type="button" [disabled]="!testPhoneInput().trim()" (click)="sendTest(selectedFlow()!, node.id)">Send</button><button type="button" (click)="testNode.set(null)">Cancel</button></div>} }</div> } } @else { <p>Select a flow to preview nodes.</p> }</article>
        </section>
      ...
      @if (activeTab() === 'Campaigns') {
        ...
        <section class="list">@for (campaign of campaigns(); track $index) { <article><div><strong>{{ campaign.name }}</strong><small>{{ campaign.templateName }}</small></div><span>{{ campaign.status }}</span>@if(confirmingCampaign()?._id === campaign._id){<div class="send-box"><small>Send to {{ preview()?.eligibleContacts || 0 }} eligible contacts?</small><button type="button" (click)="confirmCampaign(campaign); $event.stopPropagation()">Yes, send</button><button type="button" (click)="confirmingCampaign.set(null); $event.stopPropagation()">Cancel</button></div>}@else{<button type="button" [disabled]="campaign.status === 'completed' || campaign.status === 'running'" (click)="confirmingCampaign.set(campaign); $event.stopPropagation()">Confirm Send</button>}</article> }</section>
      }
  ...
  styles: [`
    ...
    .test-box{display:grid;grid-template-columns:1fr auto auto;gap:8px;margin-top:10px;text-align:left}.list .send-box{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end}.list .send-box small{color:var(--staff-text-secondary);font-size:.72rem;font-weight:700}
...
export class ShopifyAdminPage implements OnInit {
  ...
  readonly testNode = signal<{ flowId: string; nodeId: string } | null>(null);
  readonly testPhoneInput = signal("");
  readonly confirmingCampaign = signal<Campaign | null>(null);
  ...
  beginTest(flow: Flow, nodeId: string) { this.testNode.set({ flowId: flow._id, nodeId }); this.testPhoneInput.set(this.testPhone || ""); }
  connect = { shop: "", code: "" };
  testPhone = "";
  ...
  async sendTest(flow: Flow, nodeId: string) { const phone = this.testPhoneInput().trim(); if (!phone) { this.message.set("Enter a WhatsApp phone number to send the test."); return; } this.testPhone = phone; this.testNode.set(null); const result = await this.post<{ status: string; error?: string }>(`/flows/${flow._id}/test-message`, { nodeId, phone }); this.message.set(result.error ? `Test failed: ${result.error}` : `Test message ${result.status}.`); await this.refresh(); }
  ...
  async confirmCampaign(campaign: Campaign) { this.confirmingCampaign.set(null); try { await this.post(`/campaigns/${campaign._id}/send`, {}); this.message.set("Campaign send completed through approved WhatsApp templates."); } catch { this.message.set("Campaign send failed. Confirm the campaign is eligible before retrying."); } await this.refresh(); }
