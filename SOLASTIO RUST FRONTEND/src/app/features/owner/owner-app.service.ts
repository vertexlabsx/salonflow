...
import { OwnerCampaign, OwnerChatConversation, OwnerChatMessage, OwnerChatMessagesResponse, OwnerChatReceiptResponse, OwnerClient, OwnerClientDetail, OwnerInventoryDetail, OwnerInventoryProduct, OwnerInventoryResponse, OwnerNotification, OwnerNotificationReceipt, OwnerOperationsQuery, OwnerOperationsResponse } from "./owner-operations.models";
...
const OWNER_SESSION_KEY = "auraOwner:rustSession";
...
export class OwnerAppService {
  ...
  async logout(): Promise<boolean> {
    ...
    const previousRefreshToken = this.refreshToken || this.readStoredRefreshToken();
    ...
    try {
      ...
      this.invalidateSession();
      await firstValueFrom(this.http.post(`${this.baseUrl}/auth/logout`, { refreshToken: previousRefreshToken }, { headers, withCredentials: true }));
      this.setLogoutPending(false);
      return true;
    } catch {
  ...
  appointments(params: OwnerAppointmentListParams): Promise<OwnerAppointmentListResponse> {
    const query: { [key: string]: string | number | boolean } = { branchId: params.branchId, from: this.rfc3339Start(params.from), to: this.rfc3339End(params.to), limit: params.limit || 100, offset: params.offset || 0 };
  ...
  ownerPayroll(params: Record<string, string | number | boolean>): Promise<OwnerListResponse<OwnerPayroll>> { return this.get("/owner-console/people/payroll/runs", params); }
  ownerPayrollDetail(id: string): Promise<OwnerPayrollDetail> { return this.get(`/owner-console/people/payroll/runs/${encodeURIComponent(id)}`); }
  ...
  approveOwnerPayroll(id: string): Promise<OwnerPayroll> { return this.patch(`/owner-console/people/payroll/runs/${encodeURIComponent(id)}/status`, { status: "approved" }); }
  markOwnerPayrollPaid(id: string): Promise<OwnerPayroll> { return this.patch(`/owner-console/people/payroll/runs/${encodeURIComponent(id)}/status`, { status: "paid" }); }
  ...
  async ownerInventory(params: OwnerOperationsQuery & { category?: string; supplier?: string }): Promise<OwnerInventoryResponse> {
    try { return await this.get("/owner-console/operations/inventory", this.operationsParams(params)); }
    catch (error) { if (error instanceof HttpErrorResponse && error.status === 404) return this.emptyInventory(params); throw error; }
  }
  ...
  async ownerNotifications(params: OwnerOperationsQuery & { category?: string; read?: string; type?: string }): Promise<OwnerOperationsResponse<OwnerNotification>> {
    try { return await this.get("/owner-console/operations/notifications", this.operationsParams(params)); }
    catch (error) { if (error instanceof HttpErrorResponse && error.status === 404) return this.emptyOperations(params); throw error; }
  }
  ...
  async whatsappStatus(): Promise<OwnerWhatsAppStatus> {
    try { return await this.get("/whatsapp/status"); }
    catch (error) { if (error instanceof HttpErrorResponse && error.status === 404) return { configured: false, connections: [] } as OwnerWhatsAppStatus; throw error; }
  }
  ...
  private rfc3339Start(value: string): string {
    return /^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T00:00:00.000Z` : value;
  }

  private rfc3339End(value: string): string {
    return /^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T23:59:59.999Z` : value;
  }

  private emptyOperations<T>(params: { page?: number; pageSize?: number }): OwnerOperationsResponse<T> {
    const page = Number(params.page || 1);
    const pageSize = Number(params.pageSize || 30);
    return {
      items: [],
      page: { page, pageSize, total: 0, totalPages: 0, hasMore: false },
      metadata: { timezone: "Asia/Kolkata", partial: true, unavailableSources: ["backend-route-missing"], scopeNote: "This module is waiting for its Rust endpoint." }
    };
  }

  private emptyInventory(params: { page?: number; pageSize?: number }): OwnerInventoryResponse {
    return {
      ...this.emptyOperations<OwnerInventoryProduct>(params),
  ...
  private async performRefresh(): Promise<void> {
    this.refreshToken ||= this.readStoredRefreshToken();
  ...
  private applySession(session: OwnerSession): void {
    ...
    this.writeStoredRefreshToken(this.refreshToken);
  }
  ...
  private async finishPendingLogout(): Promise<void> {
    try {
      const refreshToken = this.refreshToken || this.readStoredRefreshToken();
      await firstValueFrom(this.http.post(`${this.baseUrl}/auth/logout`, { refreshToken }, { withCredentials: true }));
      this.setLogoutPending(false);
  ...
  private readStoredRefreshToken(): string {
    try { return sessionStorage.getItem(OWNER_SESSION_KEY) || ""; } catch { return ""; }
  }

  private writeStoredRefreshToken(token: string): void {
    try { token ? sessionStorage.setItem(OWNER_SESSION_KEY, token) : sessionStorage.removeItem(OWNER_SESSION_KEY); } catch { /* Session still works in memory. */ }
  }
