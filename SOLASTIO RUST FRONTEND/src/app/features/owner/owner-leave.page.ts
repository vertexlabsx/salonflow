  ...
  template: `<article class="people-page">
    ...
    <section class="data-panel">
      ...
      @if (filtered().length) {
        ...
        <div class="empty">
          <h2>{{ loading() ? 'Loading leave requests…' : 'No matching requests' }}</h2>
          <p>{{ loading() ? 'Fetching the latest leave requests.' : 'No leave requests match this branch, period and state.' }}</p>
        </div>
      }
