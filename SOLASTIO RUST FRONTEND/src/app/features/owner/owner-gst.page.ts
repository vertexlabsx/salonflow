    ...
    <article class="gst-page" [attr.aria-busy]="loading()">
      ...
      <section class="gst-grid">
        ...
        <section class="gst-card">
          ...
          <div class="expense-list">@for (expense of expenses(); track expense.id) { <article><div><strong>{{ expense.vendor || label(expense.category) }}</strong><span>{{ expense.date }} · {{ label(expense.category) }}</span><small>{{ expense.description || 'No description' }}</small></div><aside><b>{{ money(expense.totalPaise) }}</b><span>GST {{ money(expense.taxPaise) }}</span>@if(confirmingExpense()?.id === expense.id){<div class="confirm-mini"><small>Delete this expense?</small><div><button type="button" class="danger" (click)="removeExpense(expense)">Delete</button><button type="button" (click)="confirmingExpense.set(null)">Keep</button></div></div>}@else{<button type="button" (click)="editExpense(expense)">Edit</button><button type="button" class="danger" (click)="confirmingExpense.set(expense)">Delete</button>}</aside></article> } @empty { <p class="empty-state">No expenses recorded for this period.</p> }</div>
        </section>
      </section>
    </article>
  `,
...
export class OwnerGstPage {
  ...
  readonly confirmingExpense = signal<OwnerExpense | null>(null);
  ...
  async removeExpense(expense: OwnerExpense): Promise<void> {
    if (this.confirmingExpense()?.id !== expense.id) return;
    this.confirmingExpense.set(null);
