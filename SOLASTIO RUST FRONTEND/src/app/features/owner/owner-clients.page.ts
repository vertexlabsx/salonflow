...
<article class="ops-page" [attr.aria-busy]="loading()">
  ...
  <section class="ops-panel" aria-labelledby="client-results-title">
    ...
    @if (blockingError()) { <div class="ops-state" role="alert"><span>!</span><h2>Clients unavailable</h2><p>{{ blockingError() }}</p><button class="ops-button" type="button" (click)="load()">Try again</button></div> }
    @else {
      <table class="ops-table"><thead><tr><th style="width:26%">Client</th><th>Branch</th><th>Relationship</th><th>Spend</th><th>Outstanding</th><th>Status</th></tr></thead><tbody>@if (items().length) { @for(client of items();track client.id){<tr tabindex="0" (click)="open(client,$event)" (keydown.enter)="open(client,$event)" [class.selected]="detail()?.client?.id===client.id"><td><strong>{{ client.name }}</strong><small>{{ maskedContact(client) }}</small></td><td><strong>{{ client.branchName }}</strong><small>{{ client.id }}</small></td><td><strong>{{ client.visitCount }} visits</strong><small>{{ client.lastVisitAt ? date(client.lastVisitAt) : 'No recorded visit' }}</small></td><td><strong>{{ client.totalSpendPaise | paiseInr }}</strong><small>Recorded total</small></td><td><strong>{{ client.outstandingPaise | paiseInr }}</strong><small>Open invoices</small></td><td><span class="ops-status" [attr.data-tone]="client.status==='active'?'success':'neutral'">{{ client.status }}</span></td></tr>} } @else { <tr class="ops-empty-row"><td colspan="6">{{ loading() ? 'Loading clients…' : 'No matching clients. Clear or change the current filters to broaden the directory.' }}</td></tr> }</tbody></table>
      <div class="ops-cards">@if (items().length) { @for(client of items();track client.id){<button class="ops-card" type="button" (click)="open(client,$event)"><div><h3>{{ client.name }}</h3><span class="ops-status" [attr.data-tone]="client.status==='active'?'success':'neutral'">{{ client.status }}</span></div><p>{{ maskedContact(client) }} · {{ client.branchName }}</p><small>{{ client.visitCount }} visits · {{ client.totalSpendPaise | paiseInr }} spend</small></button>} } @else { <div class="ops-empty-card">{{ loading() ? 'Loading clients…' : 'No matching clients.' }}</div> }</div>
      @if (page(); as p) { <footer class="ops-pagination"><span>Page {{ p.page }} of {{ p.totalPages }}</span><button type="button" [disabled]="p.page===1||loading()" (click)="go(-1)" aria-label="Previous page">←</button><button type="button" [disabled]="!p.hasMore||loading()" (click)="go(1)" aria-label="Next page">→</button></footer> }
    }
  </section>
</article>
  ...
  <aside class="ops-drawer" role="dialog" aria-modal="true" aria-labelledby="client-detail-title" #drawer tabindex="-1">
    ...
    <div class="ops-drawer-body">
      ...
      @else if(detail();as data){
        ...
        <section class="client-quick-actions" aria-label="Client quick actions"><a [href]="phoneLink(data.client.phone)">Call</a><a [href]="whatsappLink(data.client.phone)" target="_blank" rel="noreferrer">WhatsApp</a><a [href]="emailLink(data.client.email)">Email</a>@if(reviewingId() === data.client.id){<div class="review-prompter"><small>Paste review link</small><input name="reviewUrl" [(ngModel)]="reviewUrl" placeholder="https://g.page/r/..." (keydown.enter)="sendReview(data.client.id)"><div><button type="button" class="ops-button primary" [disabled]="saving() || !reviewUrl().trim()" (click)="sendReview(data.client.id)">{{ saving() ? 'Sending...' : 'Send link' }}</button><button type="button" [disabled]="saving()" (click)="cancelReview()">Cancel</button></div></div>}@else{<button type="button" (click)="beginReview(data.client.id)">Review link</button>}</section>
...
`,
  ...
  styles: [`
    ...
    .review-prompter{grid-column:1/-1;display:grid;gap:8px;border:1px solid var(--owner-line);border-radius:13px;padding:10px;background:var(--owner-panel)}.review-prompter small{color:var(--owner-faint);font-size:.55rem;font-weight:850;letter-spacing:.05em;text-transform:uppercase}.review-prompter input{min-height:42px;border:1px solid var(--owner-line);border-radius:11px;background:var(--owner-bg);color:var(--owner-text);padding:0 10px;font:inherit}.review-prompter>div{display:flex;gap:8px;justify-content:flex-end}
    ...
    @media(max-width:600px){.client-360-hero{grid-template-columns:54px minmax(0,1fr);padding:14px}.client-360-hero .ops-button{grid-column:1/-1}.client-avatar{width:54px;height:54px;border-radius:16px;font-size:1rem}.client-signal-grid{grid-template-columns:1fr}.client-quick-actions,.photo-form{grid-template-columns:1fr 1fr}.client-quick-actions a,.client-quick-actions button{min-height:44px}.client-form{grid-template-columns:1fr}.client-form-actions{bottom:-90px}}
    @media(max-width:380px){.client-quick-actions,.photo-form{grid-template-columns:1fr}.client-360-hero{grid-template-columns:1fr}.client-avatar{width:50px;height:50px}.client-form-actions{position:static;display:grid;grid-template-columns:1fr}.client-form-actions .ops-button{width:100%}}
  `]
})
...
form = emptyClientForm();
  ...
  beginReview(clientId: string) { this.reviewingId.set(clientId); this.reviewUrl.set(""); }
  cancelReview() { this.reviewingId.set(""); this.reviewUrl.set(""); }
  async sendReview(clientId: string) {
    const reviewUrl = this.reviewUrl().trim();
    if (!reviewUrl) { this.formError.set("Paste your Google review link or public review URL."); return; }
    this.saving.set(true); this.formError.set("");
    try { await this.owner.sendOwnerReviewRequest(clientId, { reviewUrl }); this.reviewingId.set(""); this.reviewUrl.set(""); }
