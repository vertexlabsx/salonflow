  ...
  template: `
    <article class="ops-page notifications-page" [attr.aria-busy]="loading()">
      ...
      <section class="ops-panel notifications-panel">
        ...
        @if (blockingError()) { <div class="ops-state" role="alert"><span>!</span><h2>Notifications unavailable</h2><p>{{ blockingError() }}</p><button class="ops-button" type="button" (click)="load()">Try again</button></div>
        } @else {
          <table class="ops-table"><thead><tr><th style="width:40%">Notification</th><th>Category</th><th>Scope</th><th>Time</th><th>Status</th><th>Read</th></tr></thead><tbody>@if (items().length) { @for(item of items(); track item.id){<tr [class.selected]="!item.isRead"><td><strong>{{ item.message }}</strong><small>{{ item.type || 'No type' }} · {{ item.channel }}</small></td><td><span class="ops-status">{{ categoryLabel(item.category) }}</span></td><td><strong>{{ item.branchName }}</strong><small>{{ item.scope === 'tenant-wide' ? 'Branch unavailable' : 'Client-linked branch' }}</small></td><td><strong>{{ dateTime(item.createdAt) }}</strong></td><td><span class="ops-status" [attr.data-tone]="tone(item.status)">{{ statusLabel(item.status) }}</span></td><td><button class="ops-button" type="button" [disabled]="savingId() === item.id" (click)="toggle(item)">{{ savingId() === item.id ? 'Saving…' : item.isRead ? 'Mark unread' : 'Mark read' }}</button></td></tr>} } @else { <tr class="ops-empty-row"><td colspan="6">{{ loading() ? 'Loading notifications…' : 'There are no saved notifications for this filter.' }}</td></tr> }</tbody></table>
          <div class="ops-cards">@if (items().length) { @for(item of items(); track item.id){<article class="ops-card notification-card" [class.unread]="!item.isRead"><header><span class="notification-category"><i aria-hidden="true"></i>{{ categoryLabel(item.category) }}</span><span class="notification-read-state">{{ item.isRead ? 'Read' : 'New' }}</span></header><p class="notification-message">{{ item.message }}</p><div class="notification-meta"><span>{{ dateTime(item.createdAt) }}</span><span>{{ item.branchName }}</span></div><footer><span class="ops-status" [attr.data-tone]="tone(item.status)">{{ statusLabel(item.status) }}</span><button class="notification-read-action" type="button" [disabled]="savingId() === item.id" (click)="toggle(item)">{{ savingId() === item.id ? 'Saving…' : item.isRead ? 'Mark unread' : 'Mark read' }}</button></footer></article>} } @else { <div class="ops-empty-card">{{ loading() ? 'Loading notifications…' : 'There are no saved notifications for this filter.' }}</div> }</div>
          @if (page(); as p) { <footer class="ops-pagination" aria-label="Notification pages"><span>Page {{ p.page }} of {{ p.totalPages }}</span><button type="button" [disabled]="p.page === 1 || loading()" (click)="go(-1)" aria-label="Previous notification page">←</button><button type="button" [disabled]="!p.hasMore || loading()" (click)="go(1)" aria-label="Next notification page">→</button></footer>
        }
        }
      </section>
    </article>
