        ...
        <div class="grid">
          ...
          <section class="panel devices">
            <div class="panel-head"><div><b>Trusted devices</b><small>Approve registrations or revoke access</small></div><button class="button" type="button" (click)="loadDevices()">Refresh</button></div>
             @for (device of devices(); track device.id) { <article><div><strong>{{ device.staffId }}</strong><small>{{ device.deviceLabel }} · {{ device.platform }} · {{ device.publicKeyAlgorithm }}</small><small>{{ device.verificationCapability }} · hardware claim: {{ device.hardwareBackedClaim ? 'yes (client-reported)' : 'no' }} · attestation: {{ device.attestationStatus }}</small><code>{{ device.keyFingerprint }}</code></div><span [attr.data-status]="device.status">{{ label(device.status) }}</span><div class="actions">@if(confirmingDevice() === device.id){<span class="confirm-label">Confirm {{label(confirmingDecision())}}?</span><button type="button" [disabled]="busyDevice() === device.id" (click)="confirmSetDevice()">{{busyDevice() === device.id ? 'Working...' : 'Confirm'}}</button><button type="button" [disabled]="busyDevice() === device.id" (click)="confirmingDevice.set('')">Cancel</button>}@else{@if(device.status !== 'approved'){<button type="button" [disabled]="busyDevice() === device.id" (click)="requestSetDevice(device,'approved')">Approve</button>}@if(device.status !== 'revoked'){<button type="button" [disabled]="busyDevice() === device.id" (click)="requestSetDevice(device,'revoked')">Revoke</button>}}</div></article> } @empty { <p class="empty">No registered devices for this branch.</p> }
          </section>
        </div>
...
export class OwnerAttendanceVerificationComponent {
  ...
  readonly error = signal(""); readonly message = signal(""); readonly decision = signal(""); readonly confirmingDevice = signal(""); readonly confirmingDecision = signal("approved");
