...
export class OwnerContextService {
  ...
  applySettings(settings: OwnerGeneralSettings): void {
    ...
    document.documentElement.dataset["ownerCompactMode"] = merged.interface.compactMode ? "true" : "false";
document.title = `${merged.workspace.workspaceName || "Solastio"} | Owner`;
    if (!merged.branchBehavior.rememberLastBranch) this.writeStorage(this.storageKey("branch"), "");
  }

  private mergeSettings(incoming: OwnerGeneralSettings): OwnerGeneralSettings {
    return structuredClone({
      ...structuredClone(DEFAULT_SETTINGS),
      ...incoming,
      workspace: { ...DEFAULT_SETTINGS.workspace, ...(incoming.workspace || {}) },
      localization: { ...DEFAULT_SETTINGS.localization, ...(incoming.localization || {}) },
      branchBehavior: { ...DEFAULT_SETTINGS.branchBehavior, ...(incoming.branchBehavior || {}) },
      dateTime: { ...DEFAULT_SETTINGS.dateTime, ...(incoming.dateTime || {}) },
      interface: { ...DEFAULT_SETTINGS.interface, ...(incoming.interface || {}) },
      defaults: { ...DEFAULT_SETTINGS.defaults, ...(incoming.defaults || {}) },
      whatsappNudges: { ...DEFAULT_SETTINGS.whatsappNudges, ...(incoming.whatsappNudges || {}) },
      whatsappPolicy: { ...DEFAULT_SETTINGS.whatsappPolicy, ...(incoming.whatsappPolicy || {}) },
      booking: { ...DEFAULT_SETTINGS.booking, ...(incoming.booking || {}) }
    });
  }
