  ...
  template: `<article class="people-page">
    ...
    <section class="data-panel">
      ...
      @if (filtered().length) {
        ...
        <div class="empty">
          <h2>{{ loading() ? 'Loading payroll runs…' : 'No payroll runs' }}</h2>
          <p>{{ loading() ? 'Fetching the latest backend-generated runs.' : 'No backend-generated run matches the selected filters.' }}</p>
        </div>
      }
