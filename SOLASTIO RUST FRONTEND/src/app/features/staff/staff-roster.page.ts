...
@Component({
  ...
  template: `
    <section class="page roster-page">
              ...
              @for (shift of data.schedules; track shift.id) {
                <div class="row">
                  <div class="row-main">
                    ...
                    <small>{{ dayLabel(shift.scheduleDate) || 'Today' }}</small>
                  </div>
            ...
            <div class="list">
              @for (item of upcomingSchedules(); track item.id) {
                <div class="row">
                  ...
                  <div class="row-actions">
                    <span class="badge" [class.red]="item.status === 'cancelled'" [class.green]="item.status === 'completed'">{{ item.status }}</span>
                    @if (canUpdateRoster()) {
      ...
      @if (canUseShiftSwaps()) {
        <section class="grid two">
          <article class="panel"><div class="panel-title"><h2>Needs your response</h2><span>{{ incomingSwaps().length }}</span></div><div class="list">@for (swap of incomingSwaps(); track swap.id) { <div class="row"><div class="row-main"><strong>{{ dayLabel(swap.scheduleDate) }} · {{ swap.startTime }} - {{ swap.endTime }}</strong><small>{{ swap.fromStaffName || 'Coworker' }}: {{ swap.reason || 'No reason added' }}</small></div><div class="row-actions"><button class="button" type="button" [disabled]="swapBusy()" (click)="respondSwap(swap, 'accept')">Accept</button><button class="button" type="button" [disabled]="swapBusy()" (click)="respondSwap(swap, 'decline')">Decline</button></div></div> } @empty { <p class="empty">No swap request needs your response.</p> }</div></article>
          <article class="panel"><div class="panel-title"><h2>Swap requests</h2><span>{{ swaps().length }}</span></div><div class="list">@for (swap of swaps(); track swap.id) { <div class="row"><div class="row-main"><strong>{{ dayLabel(swap.scheduleDate) }} · {{ swap.startTime }} - {{ swap.endTime }}</strong><small>{{ swap.fromStaffName }} → {{ swap.toStaffName }}</small><small>{{ swapStatusLabel(swap.status) }}</small></div><div class="row-actions"><span class="badge" [class.green]="swap.status === 'approved'" [class.red]="['rejected', 'declined', 'cancelled'].includes(swap.status)">{{ swapStatusLabel(swap.status) }}</span>@if (swap.fromStaffId === staff.user()?.staffId && (swap.status === 'pending_staff' || swap.status === 'pending_manager')) { <button class="link-button" type="button" [disabled]="swapBusy()" (click)="cancelSwap(swap)">Cancel</button> }</div></div> } @empty { <p class="empty">No shift swap requests yet.</p> }</div></article>
        </section>
      }
    </section>
  `,
  ...
  styles: [`
    :host { display: block; }
    .roster-page { gap: 18px; }
    @media (max-width: 700px) {
      .roster-page { padding-inline: 12px; gap: 12px; }
      .roster-page .page-head .row-actions { flex-wrap: wrap; }
      .roster-page .page-head .row-actions input[type="date"] { flex: 1 1 100%; }
    }
  `]
})
export class StaffRosterPage implements OnInit, OnDestroy {
  ...
  dayLabel(date: string | null | undefined): string {
    if (!date) return "";
