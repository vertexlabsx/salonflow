...
@Component({
      ...
      @if (canReadCalendar()) {
        @if (view() === 'day') {
          <section class="grid two">
            ...
            <article class="panel">
              <div class="panel-title"><h2>Appointments today</h2><span>{{ os()?.timeline?.length || 0 }}</span></div>
              <div class="list">
                @for (item of os()?.timeline?.slice(0, 6) || []; track item.id) {
                  <div class="row"><div class="row-main"><strong>{{ item.serviceNames.join(', ') || 'Assigned appointment' }}</strong><small>{{ item.startAt | date:'shortTime' }} · {{ item.state }}</small></div><span class="badge">{{ item.status }}</span></div>
                } @empty { <p class="empty">No appointments on the selected day.</p> }
              </div>
            </article>
          </section>
        } @else {
          <section class="calendar-week">
            @for (item of weekSchedules(); track item.id) {
              <article class="panel" [draggable]="canUpdateCalendar()" (dragstart)="dragSchedule(item)" (dragover)="$event.preventDefault()" (drop)="dropSchedule(item.date)">
                <div class="panel-title"><h2>{{ calendarDayLabel(item.date) }}</h2><span>{{ item.status }}</span></div>
  ...
  styles: [`.icon-button{width:48px;padding:0}.icon-button svg{width:18px;height:18px;fill:currentColor}@media(max-width:700px){.calendar-page .page-head{display:grid;align-items:start}.calendar-page .page-head .row-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.calendar-page .page-head .row-actions input{grid-column:1/-1}.calendar-page .page-head .row-actions .icon-button{width:100%}}`]
})
export class StaffCalendarPage implements OnInit, OnDestroy {
  ...
  calendarDayLabel(date: string): string {
    ...
    if (parts.length !== 3 || parts.some((part) => Number.isNaN(part))) return date || "Scheduled";
    return new Intl.DateTimeFormat("en", { weekday: "short", day: "numeric", month: "short", timeZone: "Asia/Kolkata" }).format(new Date(Date.UTC(parts[0], parts[1] - 1, parts[2])));
  }
