...
@Component({
  ...
  template: `
    <section class="page"><header class="page-head"><div><p class="eyebrow">Performance</p><h1>Performance intelligence</h1><p>Productivity, utilization, rating and improvement signals.</p></div></header>
      ...
      @if (os(); as data) {
        <section class="grid four"><article class="kpi"><span>Score</span><strong>{{ data.performance.productivityScore }}/100</strong><small>Overall tracking score</small></article><article class="kpi"><span>Services</span><strong>{{ data.performance.completedServices }}</strong><small>Services completed</small></article><article class="kpi"><span>Utilization</span><strong>{{ data.performance.avgUtilization }}%</strong><small>Capacity utilization</small></article><article class="kpi"><span>Rating</span><strong>{{ data.performance.avgRating || '-' }}</strong><small>Average client rating</small></article></section>
        <section class="panel"><div class="panel-title"><h2>Trend board</h2><span>daily to yearly</span></div><div class="trend-grid">@for (key of reportKeys(); track key) { <article><span>{{ reportLabel(key) }}</span><strong>{{ data.reports[key].productivityScore }}/100</strong><div class="timer-track"><span [style.width.%]="data.reports[key].productivityScore"></span></div><small>{{ data.reports[key].services }} services · {{ data.reports[key].rating || 0 }} rating</small></article> }</div></section>
        ...
        @if (canSeeRevenue()) { <section class="panel"><div class="panel-title"><h2>Revenue impact</h2><span>connected</span></div><h2 class="revenue-total">{{ data.performance.revenue | paiseInr }}</h2><small class="revenue-note">Revenue attributed to your completed services across the period.</small></section> }
      }
    </section>
  `,
  ...
  styles: [`
    :host { display: block; }
    .revenue-total { margin: 6px 0 0; font-size: 1.8rem; font-weight: 840; letter-spacing: -.03em; background: linear-gradient(120deg, var(--staff-primary), var(--staff-primary-hover)); -webkit-background-clip: text; background-clip: text; color: transparent; }
    .revenue-note { display: block; margin-top: 4px; color: var(--staff-text-secondary); font-size: .78rem; font-weight: 650; }
    @media (max-width: 700px) { .performance-grid, .grid.four { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
  `]
})
export class StaffPerformancePage implements OnInit {
  ...
  reportLabel(key: string): string { return ({ daily: "Daily", weekly: "Weekly", monthly: "Monthly", yearly: "Yearly" } as Record<string, string>)[key] || key; }
}
