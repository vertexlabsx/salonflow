...
@Component({
  ...
  styles: [`
    ...
    .biometric-switch { position: relative; width: 50px; min-width: 50px; height: 32px; min-height: 44px; flex: 0 0 50px; padding: 5px 0; border: 1px solid var(--staff-border-accent); border-radius: 999px; background: var(--staff-surface-secondary); cursor: pointer; transition: background-color 180ms ease, border-color 180ms ease; }
    .biometric-switch span { position: absolute; top: 50%; left: 6px; width: 20px; height: 20px; border-radius: 50%; background: var(--staff-text-secondary); transform: translateY(-50%); transition: transform 180ms ease, background-color 180ms ease; }
    .biometric-switch[aria-checked="true"] { border-color: var(--staff-primary); background: var(--staff-primary); }
    .biometric-switch[aria-checked="true"] span { transform: translate(18px, -50%); background: var(--staff-on-primary); }
