...
@Component({
  ...
  styles: [`
    ...
    .kanban-card.overdue { outline: 1px solid color-mix(in srgb, var(--staff-danger, #c0392b) 40%, transparent); outline-offset: -1px; }
    .task-overdue { display: inline-block; margin-left: 7px; padding: 2px 7px; border-radius: 999px; background: var(--staff-error-surface); color: var(--staff-error-text); font-size: .6rem; font-weight: 800; text-transform: uppercase; letter-spacing: .04em; }
    ...
    @media (max-width: 420px) {
      .task-summary { display:grid; grid-template-columns:1fr; align-items:start; }
      .task-summary-progress { width:100%; max-width:none; }
      .task-summary-progress > span { text-align:left; }
      .kanban-card .row-actions { justify-content:flex-start; }
      .kanban-card .row-actions > * { flex:1 1 auto; }
    }
...
export class StaffTasksPage implements OnInit {
  ...
  isOverdue(task: { status?: string | null; dueAt?: string | null }): boolean {
    if (!task.dueAt || !task.status || task.status === "completed" || task.status === "in_progress") return false;
