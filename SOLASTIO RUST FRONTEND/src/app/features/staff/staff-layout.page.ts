...
@Component({
    ...
    <section class="staff-app-shell" [class.staff-compact]="preferences().interface.compactMode">
      ...
      <aside class="staff-sidebar" [class.open]="menuOpen()" [attr.role]="menuOpen() ? 'dialog' : null" [attr.aria-modal]="menuOpen() ? 'true' : null" [attr.aria-label]="menuOpen() ? 'Staff navigation' : null" [attr.inert]="notificationsOpen() || commandOpen() ? '' : null" tabindex="-1" #menuDialog (keydown)="menuOpen() && trapFocus($event, menuDialog)">
       ...
       <nav class="mobile-bottom-nav" aria-label="Primary staff navigation" [attr.inert]="menuOpen() || notificationsOpen() || commandOpen() ? '' : null">
          ...
          <button type="button" class="mobile-more" (click)="openMenu()" aria-label="Open all staff tools" [attr.aria-expanded]="menuOpen()"><svg viewBox="0 0 24 24" aria-hidden="true"><path [attr.d]="iconFor('More')"></path></svg><span>More</span></button>
       </nav>
  ...
  `,
  styles: [`
    .staff-app-shell { min-height: 100vh; display: grid; grid-template-columns: 292px minmax(0, 1fr); background: radial-gradient(circle at 0 0,color-mix(in srgb,var(--staff-primary) 11%,transparent),transparent 26%), radial-gradient(circle at 100% 0,color-mix(in srgb,var(--staff-border-accent) 22%,transparent),transparent 24%), var(--staff-background); color: var(--staff-text); }
    .staff-sidebar { position: sticky; top: 0; height: 100vh; overflow: auto; padding: 18px; border-right: 1px solid rgba(217,180,111,.24); background: radial-gradient(circle at 18% 0, rgba(217,180,111,.18), transparent 28%), linear-gradient(180deg,#102f2d,#071f1d); color: #f9f6ef; box-shadow: 18px 0 44px rgba(23,32,31,.13); }
    .brand-card { position:relative;overflow:hidden;padding: 18px; border: 1px solid rgba(217,180,111,.34); border-radius: 24px; color: #f9f6ef; background: radial-gradient(circle at 92% 0, rgba(217,180,111,.35), transparent 38%),linear-gradient(135deg,rgba(255,255,255,.08),rgba(255,255,255,.02)); box-shadow:0 20px 42px rgba(0,0,0,.16); }
    .brand-card span { display: block; color: #d9b46f; font-size: .55rem; font-weight: 850; letter-spacing: .14em; text-transform: uppercase; }
    .brand-card strong { display: block; margin-top: 4px; font-family:Inter,ui-sans-serif,system-ui,sans-serif;font-size: 1.28rem; font-weight:820;line-height: 1.05;letter-spacing:-.045em; }
    .brand-card small { display: block; margin-top: 4px; color: rgba(249,246,239,.68); font-size: .62rem; font-weight: 650; line-height: 1.15; text-transform: capitalize; }
    ...
    .user-card { display: grid; grid-template-columns: 44px 1fr; gap: 10px; align-items: center; margin-top: 14px; padding: 11px; border: 1px solid rgba(255,255,255,.1); border-radius: 20px; background: rgba(255,255,255,.06); color: #f9f6ef; text-decoration: none; cursor: pointer; box-shadow:none; }
    .user-card:hover, .user-card:focus-visible { border-color: rgba(217,180,111,.62); background: rgba(217,180,111,.12); }
    .user-card b, .profile-avatar { display: grid; place-items: center; width: 42px; height: 42px; border-radius: 15px; background: linear-gradient(135deg,#d9b46f,#f4dfad); color: #082926; box-shadow:0 10px 22px rgba(217,180,111,.2); }
    .user-card strong, .user-card small { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .user-card small { color: rgba(249,246,239,.66); font-weight: 600; }
    ...
    .nav-group { margin: 14px 2px 5px; color: rgba(249,246,239,.48); font-size: .64rem; font-weight: 820; letter-spacing: .14em; text-transform: uppercase; }
    ...
    nav a { display: grid; grid-template-columns: 34px 1fr; gap: 10px; align-items: center; min-height:48px;padding: 8px 10px; border: 1px solid transparent; border-radius: 17px; color: rgba(249,246,239,.68); font-weight: 760; text-decoration: none; }
    nav a span { display: grid; place-items: center; width: 32px; height: 32px; border-radius: 12px; background: rgba(255,255,255,.06); color: rgba(249,246,239,.66); font-size: .7rem; font-weight: 800; }
    svg { width: 17px; height: 17px; fill: currentColor; }
    nav a.active, nav a:hover { border-color: rgba(217,180,111,.42); background: linear-gradient(135deg,rgba(217,180,111,.18),rgba(255,255,255,.07)); color: #fffdfa; box-shadow:0 10px 22px rgba(0,0,0,.12); }
    nav a.active span { background: #d9b46f; color: #082926; }
    ...
    .staff-topbar { position: relative; display: flex; justify-content: space-between; align-items: center; gap: 10px; min-height:var(--staff-header-height);padding: 10px 22px; border-bottom: 1px solid color-mix(in srgb,var(--staff-primary) 10%,var(--staff-border)); background: rgba(255,253,250,.86); backdrop-filter: blur(22px); box-shadow:0 10px 28px rgba(23,32,31,.05); }
    ...
    .search-button, .chat-button, .bell-button { border: 1px solid color-mix(in srgb,var(--staff-primary) 14%,var(--staff-border)); background:var(--staff-surface);color:var(--staff-text-secondary);font-weight:750;box-shadow:0 8px 18px rgba(23,32,31,.05); }
    ...
    .mobile-bottom-nav { display: none; }
     @media (max-width: 900px) {
       ...
       .staff-main-shell { display: block; height: 100dvh; min-height: 100dvh; overflow-y: auto; overflow-x: clip; scroll-padding-top: var(--staff-header-height); overscroll-behavior-y: contain; -webkit-overflow-scrolling: touch; }
         .staff-topbar { position: sticky; top: 0; z-index: 20; min-width:0; min-height: var(--staff-header-height); padding: calc(7px + env(safe-area-inset-top)) max(10px, env(safe-area-inset-right)) 7px max(10px, env(safe-area-inset-left)); gap: 6px; }
       .menu-button { display: inline-flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px; flex: 0 0 44px; width: 44px; height: 44px; margin: 0; padding: 0; border: 1px solid color-mix(in srgb,var(--staff-primary) 12%,var(--staff-border)); border-radius: 15px; background: var(--staff-surface); color: var(--staff-text); font-size: .78rem; font-weight: 750; box-shadow: 0 8px 18px rgba(23,32,31,.05); }
      .staff-topbar > div:nth-child(2) { min-width: 0; flex: 1 1 auto; }
        .staff-identity { flex: 1 1 auto; width:0; max-width:none; gap: 8px; overflow: hidden; }
      ...
      .staff-topbar p { font-size: .66rem; }
       ...
       .search-button { display:inline-grid;grid-template-columns:1fr;place-items:center;width:40px;height:44px;min-width:40px;padding:0;border:0;border-radius:12px;background:transparent;box-shadow:none; }
      ...
      .topbar-actions button { padding:0; }
        .topbar-actions :is(.chat-button,.bell-button) { width:40px;height:44px;min-width:40px;padding:0;border:0;border-radius:12px;background:transparent;box-shadow:none; }
      ...
      .bell-icon { width: 19px; height: 19px; }
       ...
       .staff-policy-hint { margin: 0 10px 8px; padding: 7px 10px; font-size: .72rem; line-height: 1.3; }
         .notification-drawer { top: 0; right: 0; bottom: 0; left: auto; width: min(420px, 92vw); min-width: 280px; max-width: none; height: 100dvh; padding: calc(14px + env(safe-area-inset-top)) calc(14px + env(safe-area-inset-right)) calc(14px + env(safe-area-inset-bottom)) calc(14px + env(safe-area-inset-left)); border-left: 1px solid var(--staff-border); border-radius: 26px 0 0 26px; box-shadow: -22px 0 54px rgba(23,32,31,.2); }
      .notification-drawer .drawer-title { position: sticky; top: 0; z-index: 2; border: 1px solid var(--staff-border); border-radius: 16px; background: var(--staff-surface-secondary); box-shadow: 0 6px 16px rgba(31, 41, 55, .08); }
        .mobile-bottom-nav { position: fixed; left: 50%; bottom: calc(var(--staff-mobile-nav-offset) + env(safe-area-inset-bottom)); z-index: 27; display: grid; grid-template-columns: repeat(auto-fit, minmax(0, 1fr)); width: min(calc(100vw - 16px), 430px); min-height: var(--staff-mobile-nav-height); padding: 7px; gap: 2px; transform: translateX(-50%); border: 1px solid color-mix(in srgb,var(--staff-primary) 18%,var(--staff-border)); border-radius: 26px; background: linear-gradient(135deg,color-mix(in srgb,var(--staff-surface-glass) 96%,transparent),color-mix(in srgb,var(--staff-surface-secondary) 88%,transparent)); box-shadow: var(--staff-shadow-elevated); backdrop-filter: blur(22px); }
       .mobile-bottom-nav:has(> :nth-child(5)) { grid-template-columns: repeat(auto-fit, minmax(0, 1fr)); gap: 1px; }
        .mobile-bottom-nav :is(a, .mobile-more) { position:relative;display: grid; grid-template-columns: 1fr; grid-template-rows: 23px auto; place-items: center; align-content: center; gap: 2px; min-width: 0; min-height:56px; padding: 6px 2px; border: 0; border-radius: 16px; color: var(--staff-text-secondary); font-size: clamp(.52rem, 2.55vw, .62rem); font-weight: 700; line-height: 1; text-decoration: none; background: transparent; cursor: pointer; transition:transform var(--staff-motion-fast) var(--staff-motion-ease),opacity var(--staff-motion-fast) var(--staff-motion-ease); } .mobile-bottom-nav :is(a, .mobile-more) span { display: block; max-width: 100%; width: auto; height: auto; padding: 0; border: 0; border-radius: 0; background: transparent; color: inherit; font-size: inherit; font-weight: inherit; letter-spacing: -.02em; text-transform: none; overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }
      ...
      .mobile-bottom-nav :is(a, .mobile-more) svg { display: block; width: 20px; height: 20px; margin: 0; fill: currentColor; }
        ...
        .mobile-bottom-nav :is(a, .mobile-more):focus-visible { outline: 3px solid var(--staff-focus-ring); outline-offset: 2px; border-radius: 14px; }
        .mobile-bottom-nav .mobile-more:active { transform: scale(.97); opacity: .8; }
      ...
      .drawer-backdrop.open { opacity: 1; pointer-events: auto; }
       .staff-sidebar { position: fixed; left: 0; top: 0; bottom: 0; z-index: 30; width: min(330px, 88vw); min-width: 0; max-width: none; box-sizing: border-box; height: 100dvh; overflow: auto; padding: calc(14px + env(safe-area-inset-top)) calc(14px + env(safe-area-inset-right)) calc(14px + env(safe-area-inset-bottom)) calc(14px + env(safe-area-inset-left)); border-right: 1px solid rgba(217,180,111,.24); border-radius: 0 28px 28px 0; transform: translateX(-104%); transition: transform .2s ease; box-shadow: 22px 0 54px rgba(23,32,31,.2); }
      ...
      @media (max-width: 560px) {
       ...
       .staff-topbar strong { display: block; max-width: none; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
       .profile-avatar { width:34px;height:34px;border-radius:12px; }
       .staff-identity span,.staff-identity small { display:none; }
      ...
      nav a { padding: 12px 13px; }
       ...
       @media (max-width: 380px) {
          ...
          .staff-identity strong { max-width:32vw;font-size:.82rem; }
          .staff-identity small { max-width:100%; }
          .topbar-actions :is(.search-button,.chat-button,.bell-button) { width:36px;min-width:36px; }
          .mobile-bottom-nav { width:calc(100vw - 10px);padding-inline:4px;border-radius:20px; }
          .mobile-bottom-nav :is(a, .mobile-more) { min-height:52px;font-size:.5rem; }
          .mobile-bottom-nav :is(a, .mobile-more) svg { width:18px;height:18px; }
        }
      @media (max-width: 420px) {
        .command-backdrop { align-items:end;place-items:end stretch;padding:0; }
        .command-palette { width:100vw;max-height:min(82dvh, 680px);border-radius:24px 24px 0 0;border-inline:0;border-bottom:0; }
        .notification-drawer { left:0;width:100vw;min-width:0;border-left:0;border-radius:24px 24px 0 0; }
        .staff-toast { bottom:calc(var(--staff-bottom-clearance) - 4px); }
      }
...
export class StaffLayoutPage implements OnInit, OnDestroy {
  ...
  private readonly nav: StaffNavItem[] = [
    { label: "Dashboard", path: "/staff/dashboard", iconPath: "M4 13h6V3H4v10zm0 8h6v-6H4v6zm10 0h6V11h-6v10zm0-18v6h6V3h-6z", group: "Home", permission: "read:appointments" },
    ...
    { label: "Queue", path: "/staff/queue", iconPath: "M4 5h16v2H4V5zm0 6h16v2H4v-2zm0 6h11v2H4v-2z", group: "Work", permission: "read:appointments" },
    ...
    { label: "Chat", path: "/staff/chat", iconPath: "M4 4h16v12H7l-3 3V4zm4 5h8V7H8v2zm0 4h6v-2H8v2z", group: "Work", permission: "read:staff" },
    { label: "Performance", path: "/staff/performance", iconPath: "M3 17h3v4H3v-4zm5-6h3v10H8V11zm5 3h3v7h-3v-7zm5-9h3v16h-3V5z", group: "Growth", permission: "read:staff" },
    { label: "Leaderboard", path: "/staff/leaderboard", iconPath: "M7 21h10v-2H7v2zM5 3h14v4a7 7 0 0 1-6 6.9V17h-2v-3.1A7 7 0 0 1 5 7V3zm2 2v2a5 5 0 0 0 10 0V5H7z", group: "Growth", permission: "read:staff" },
    { label: "Reports", path: "/staff/reports", iconPath: "M5 3h11l3 3v15H5V3zm10 1.5V7h2.5L15 4.5zM8 11h8v2H8v-2zm0 4h8v2H8v-2z", group: "Growth", permission: "read:staff" },
    ...
    { label: "Notifications", path: "/staff/notifications", iconPath: "M18 10.8c0-3.5-2.1-6.1-5-6.7V3a1 1 0 0 0-2 0v1.1c-2.9.6-5 3.2-5 6.7V15l-1.6 2.4A1 1 0 0 0 5.2 19h13.6a1 1 0 0 0 .8-1.6L18 15v-4.2zM9.7 20a2.4 2.4 0 0 0 4.6 0H9.7z", group: "Account" },
  ...
  onTouchEnd(event: TouchEvent) {
    ...
    if (this.staff.hasPermission("read:appointments")) {
      paths.push("/staff/dashboard", "/staff/appointments");
    }
