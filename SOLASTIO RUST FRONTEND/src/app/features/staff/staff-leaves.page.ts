...
@Component({
    ...
    <section class="page leaves-page">
      ...
      @if (canReadLeaves()) {
        <section class="grid two">
          <article class="panel"><div class="panel-title"><h2>Leave balance</h2><span>{{ balances().length }}</span></div><div class="list">@for (balance of balances(); track balance.id) { <div class="row"><div class="row-main"><strong>{{ balance.leaveType }}</strong><small>{{ balance.used || 0 }} used of {{ balance.openingBalance || 0 }}{{ balance.accrued ? ' + ' + balance.accrued + ' accrued' : '' }}</small></div><span class="badge" [class.green]="leaveBalanceValue(balance) > 0">{{ leaveBalanceValue(balance) }} left</span></div> } @empty { <p class="empty">No leave balances configured.</p> }</div></article>
          <article class="panel"><div class="panel-title"><h2>Leave history</h2><span>{{ leaves().length }}</span></div><div class="list">@for (leave of leaves(); track leave.id) { <div class="row"><div class="row-main"><strong>{{ leave.leaveType }} · {{ leave.days || 1 }}d</strong><small>{{ leave.startDate | date:'mediumDate' }} - {{ leave.endDate | date:'mediumDate' }}</small></div><span class="badge" [class.green]="leave.status === 'approved'" [class.red]="leave.status === 'rejected'">{{ leave.status }}</span></div> } @empty { <p class="empty">No leave requests yet.</p> }</div></article>
        </section>
        <section class="panel">
          ...
          <div class="leave-actions"><button class="button primary" type="button" [disabled]="!canRequestLeave() || submitting()" (click)="requestLeave()">{{ submitting() ? 'Sending...' : 'Send request' }}</button></div>
        </section>
      }
    </section>`,
  ...
  styles: [`
    :host { display: block; }
    .leaves-page { gap: 18px; }
    .leave-actions { display: flex; justify-content: flex-end; margin-top: 14px; }
    .leave-actions .button { min-width: 160px; }
    @media (max-width: 700px) { .leave-actions .button { width: 100%; } }
  `]
})
