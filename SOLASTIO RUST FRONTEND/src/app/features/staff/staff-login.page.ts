...
@Component({
  ...
  template: `
    <ion-content class="staff-login-shell">
      <main class="login-grid">
        <section class="login-story" aria-label="Solastio staff workspace preview">
          <p>Staff Console</p>
          <h1>Run the day from one calm, connected workspace.</h1>
          <div class="story-metrics" aria-hidden="true">
            <span><b>Live</b> appointments</span>
            <span><b>Secure</b> team access</span>
            <span><b>Synced</b> client context</span>
          </div>
        </section>
  ...
  `,
  styles: [`
    .staff-login-shell { --background: radial-gradient(circle at 12% 5%, color-mix(in srgb,var(--staff-primary) 16%,transparent), transparent 30%), radial-gradient(circle at 88% 12%, color-mix(in srgb,var(--staff-border-accent) 38%,transparent), transparent 26%), var(--staff-background); }
    .login-grid { width: min(1100px, calc(100% - 32px)); min-height: 100%; margin: 0 auto; padding: 7vh 0; display: grid; grid-template-columns: minmax(0, 1fr) minmax(380px, 500px); gap: 28px; align-items: stretch; }
    .login-story { position: relative; overflow: hidden; display: grid; align-content: end; gap: 24px; min-height: 620px; padding: 42px; border: 1px solid rgba(217,180,111,.34); border-radius: 38px; color: #f9f6ef; background: radial-gradient(circle at 70% 12%, rgba(217,180,111,.32), transparent 34%), linear-gradient(135deg,#123c3a,#071f1d); box-shadow: var(--staff-shadow-elevated); }
    .login-story::before { content:""; position:absolute; inset:30px 30px auto auto; width:190px; height:190px; border:1px solid rgba(217,180,111,.48); border-radius:999px; background:repeating-linear-gradient(135deg,transparent 0 13px,rgba(255,255,255,.08) 13px 14px); }
    .login-story p { margin:0; color:#d9b46f; font-size:.72rem; font-weight:850; letter-spacing:.16em; text-transform:uppercase; }
    .login-story h1 { position:relative; z-index:1; max-width:720px; margin:0; font-size:clamp(3rem,6vw,5.9rem); line-height:.9; letter-spacing:-.085em; }
    .story-metrics { position:relative; z-index:1; display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:10px; }
    .story-metrics span { display:grid; gap:4px; min-height:86px; padding:14px; border:1px solid rgba(255,255,255,.12); border-radius:20px; background:rgba(255,255,255,.06); color:rgba(249,246,239,.68); font-size:.78rem; font-weight:700; }
    .story-metrics b { color:#fffdfa; font-size:1rem; }
    .staff-card { position: relative; overflow: hidden; align-self:center; padding: 38px; border: 1px solid color-mix(in srgb,var(--staff-primary) 18%,var(--staff-border)); border-radius: 34px; background: linear-gradient(145deg,rgba(255,255,255,.98),var(--staff-surface-secondary)); box-shadow: var(--staff-shadow-elevated); }
    ...
    h2 { margin: 0; color: var(--staff-text); font-family:Inter,ui-sans-serif,system-ui,sans-serif;font-size: clamp(2.2rem, 5vw, 3.45rem); font-weight:840;line-height: .94; letter-spacing: -.075em; }
    ...
    @media (max-width: 920px) { .login-grid { width: calc(100% - 30px); grid-template-columns:1fr; padding: max(20px, env(safe-area-inset-top)) 0 max(20px, env(safe-area-inset-bottom)); } .login-story { display:none; } .staff-card { padding: 24px 20px;border-radius:28px; } }
    @media (max-width: 420px) { .login-grid { width: 100%; min-height: 100dvh; padding: max(12px, env(safe-area-inset-top)) 12px max(12px, env(safe-area-inset-bottom)); align-items: center; } .staff-card { display:grid;align-content:start;min-height:0;padding:18px 14px;border-radius:18px;box-shadow:0 10px 28px rgba(23,32,31,.12); } h2 { font-size: clamp(1.45rem, 8vw, 1.9rem); line-height:1.05; } .staff-form { gap: 9px; margin-top: 14px; } .floating-field input { height: 50px; min-height:50px; padding-left:44px; padding-right:44px; } .floating-field label { left:44px; max-width:calc(100% - 88px); } .floating-field > svg { left:14px; } button,.biometric-button,.secondary-button { min-height:46px;margin-top:8px; } }
