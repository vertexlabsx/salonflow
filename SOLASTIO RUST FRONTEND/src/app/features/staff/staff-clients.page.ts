...
@Component({
    ...
    <section class="page clients-page">
      ...
      @if (detail(); as data) {
        <article class="panel dark client-hero">
          ...
          <div class="hero-actions">@if (staff.hasPermission('update:clients')) { <button type="button" class="button primary" (click)="edit(data)">Edit client</button> }<button type="button" class="button" (click)="clearDetail()">Back to directory</button></div>
        </article>
  ...
  styles: [`
    ...
    .client-hero h2 { margin:0; color:inherit; font-family:Inter,ui-sans-serif,system-ui,sans-serif; font-size:clamp(1.8rem,4vw,3rem); font-weight:840; letter-spacing:-.07em; }
    ...
    .client-kpis .kpi { border-color:rgba(217,180,111,.34); background:rgba(255,255,255,.08); box-shadow:none; }
    .client-kpis .kpi span,.client-kpis .kpi small { color:rgba(249,246,239,.7); }
    .client-kpis .kpi strong { color:#fffdfa; }
    ...
    @media (max-width:860px){.client-search{grid-template-columns:1fr}.client-search button{width:100%;}.client-hero{grid-template-columns:1fr;}.hero-actions{display:grid;grid-template-columns:1fr 1fr}.hero-actions .button{width:100%}.client-form-grid{grid-template-columns:1fr;}.client-form-grid .wide{grid-column:auto;}}
...
export class StaffClientsPage {
  ...
  clearDetail() { this.detail.set(null); this.mode.set("view"); }
  ...
  async save(event: Event) {
    ...
    try {
      ...
      if (this.mode() === "create") {
        ...
        const created = await this.staff.createClient({ ...payload, branchId, phone: this.form.phone });
        this.formError.set("");
        this.mode.set("view");
        await this.open(event, created.id);
        await this.search();
      } else {
