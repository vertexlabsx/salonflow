...
@Component({
  ...
  template: `
    <section class="page notifications-page">
      ...
      @if (canReadNotifications() && os(); as data) {
        <section class="panel">
          <div class="panel-title"><h2>Inbox</h2><span>{{ data.notifications.length }} total@if (unreadCount()) { · {{ unreadCount() }} unread }</span></div>
          ...
          <div class="list">
            @for (note of data.notifications; track note.id) {
              <div class="row"><div class="row-main"><strong>{{ note.title }}</strong><small>{{ note.body || 'No details' }} · {{ note.createdAt ? (note.createdAt | date:'short') : '' }}</small></div><div class="row-actions"><span class="badge" [class.badge-unread]="note.status === 'unread'" [class.badge-archived]="note.status === 'archived'">{{ note.status }}</span>@if (note.status === 'archived') { <button class="link-button" type="button" [disabled]="!canUpdateNotifications()" (click)="mark(note.id, 'unread')">Restore</button> } @else { <button class="link-button" type="button" [disabled]="!canUpdateNotifications()" (click)="mark(note.id, note.status === 'read' ? 'unread' : 'read')">{{ note.status === 'read' ? 'Unread' : 'Read' }}</button><button class="link-button" type="button" [disabled]="!canUpdateNotifications()" (click)="mark(note.id, 'archived')">Archive</button> }</div></div>
  ...
  styles: [`
    .badge.badge-unread { border: 1px solid color-mix(in srgb, var(--staff-primary) 45%, transparent); background: var(--staff-primary-light); color: var(--staff-primary-hover); }
    .badge.badge-archived { background: var(--staff-surface); color: var(--staff-text-tertiary); }
  `]
})
export class StaffNotificationsPage implements OnInit {
  ...
  unreadCount(): number { return (this.os()?.notifications || []).filter((note) => note.status === "unread").length; }
